...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  return (
    <div className="space-y-4 pb-8">
      ...
      <div id="tour-welcome" className="bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-950 rounded-2xl p-6 text-white shadow-xl relative overflow-hidden border border-white/10">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <button
            ...
            className="flex items-center space-x-2 bg-white text-slate-900 hover:bg-blue-50 px-4 py-2.5 rounded-xl font-bold text-xs transition-all shadow-lg hover:scale-105 active:scale-95"
          >
      ...
      <div id="tour-kpis" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden group">
          ...
          <div className="mt-2 flex items-center text-[9px] font-bold text-slate-500">
            <span className="bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded mr-1.5">{items.length}</span>
            REGISTROS
          </div>
        </div>

        <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm group">
      ...
      <div id="tour-charts" className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
                ...
                <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                  ...
                  <td className="py-2 px-5">
                    <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold ${
                      item.status === 'Recebida' ? 'bg-emerald-50 text-emerald-600' : 
                      item.status === 'Confirmada' ? 'bg-blue-50 text-blue-600' : 
