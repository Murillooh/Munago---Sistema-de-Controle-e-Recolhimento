...
interface TableManagerViewProps {
  ...
  onImportBulk: (items: RecolhimentoItem[]) => Promise<{ ok: true } | { ok: false; error: string }>;
  unidades: Unidade[];
...
export const TableManagerView = forwardRef<any, TableManagerViewProps>(function TableManagerView(props, ref) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (parsed.length > 0) {
        const result = await onImportBulk(parsed);
        if (result.ok === false) {
          showToast('error', `Falha ao salvar no servidor — nada foi importado: ${result.error}`);
        } else {
